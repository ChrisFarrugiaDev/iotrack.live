<template>
  <nav class="vpager" aria-label="Pagination navigation" >
    <!-- Prev -->
    <button
      class="vpager__btn vpager__btn--nav"
      :disabled="page === 1"
      @click="go(page - 1)"
    ...
    <template v-for="(p, i) in pagesToShow(page, pageCount)" :key="`p-${p}`">
      <span
        v-if="i > 0 && isGap(pagesToShow(page, pageCount)[i - 1], p)"
        class="vpager__ellipsis"
        aria-hidden="true"
      >…</span>

      <button
        class="vpager__btn vpager__btn--page"
        :class="{ 'vpager__btn--active': p === page }"
        @click="go(p)"
...
.vpager__btn {
  ...
  &:hover:not([disabled]) { background: var(--color-zinc-100); }
  &[disabled] { opacity: .5; cursor: not-allowed; }

  /* Page button modifier */
  &--page { /* reserved for page-specific tweaks if needed */ }
  ...
  /* Prev/Next modifier */
  &--nav { /* reserved for nav-specific tweaks if needed */ }
}
