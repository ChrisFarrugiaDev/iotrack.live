...
class UserAssignmentController {
    ...
    static async getAssignableResources(request: FastifyRequest, reply: FastifyReply) {
        ...
        try {
            ...
            const accessibleOrgIds = await AccessProfileController.computeAccessibleOrganisationIds(org_id, userID);
            ...
            const assets = await AccessProfileController.getAccessibleAssetsForUser(userID, accessibleOrgIds);
            const devices = await AccessProfileController.getAccessibleDevicesForUser(userID, accessibleOrgIds);
