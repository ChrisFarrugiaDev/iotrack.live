...
description: Verify computation.server.go against the LIVE environment — the PostgreSQL at 57.129.22.122:5436 is the PRODUCTION database (there is no separate dev DB yet). Use whenever you need to run or smoke-test the computation server locally, mint a test JWT for any role, hit /compute/* endpoints with curl, run SQL against it (there is no local psql and no local Docker DB), inspect permissions, or walk the Phase 1 acceptance matrix. Reach for this before hand-writing any token-minting Python, DB_URL host-swapping, background go run, or throwaway pgx query programs — the bundled scripts already do all of that.
---

# Compute Dev Check

Scripted verification for `computation.server.go`.

**The PostgreSQL at `57.129.22.122:5436` is the PRODUCTION database.** There
is no separate dev DB yet (building a sandbox from it is a recorded backlog
item). The service env files say `127.0.0.1` because production runs on that
box. Rules of engagement: reads are fine (mind heavy scans on the telemetry
hypertable); **writes only when deliberate, idempotent, and approved by the
owner** — never as test setup. These scripts do the host swap, secret
loading, lifecycle, and SQL access so none of it is re-derived by hand.
...
done** — the server holds connections to the PRODUCTION database.
...
stderr. Non-SELECT statements work too (used once for the approved
report.view seed), **but this is PRODUCTION — treat every non-SELECT as a
production change: deliberate, idempotent, owner-approved.**
